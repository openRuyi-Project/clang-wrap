...
use clang_wrap::{
    compute_llvmir_path, compute_llvmir_target_dir, debug_log, ensure_dir_exists, find_llvmir_file,
    get_exe_path, get_llvm_ir_dir, get_program_name, is_debug_mode, resolve_cmd_name,
...
fn execute_ln(ln_path: &Path, args: &[String], debug: bool) -> i32 {
    ...
    let status = Command::new(ln_path)
        .args(args)
        .status()
        .unwrap_or_else(|_| panic!("Failed to execute {}", ln_path.display()));
...
fn resolve_single_link(target: &Path, link_name_or_dir: Option<&Path>) -> PathBuf {
    let target_name = target.file_name().expect("Target should have a filename");

    match link_name_or_dir {
        Some(link_name) if link_name.is_dir() => link_name.join(target_name),
        Some(link_name) => link_name.to_path_buf(),
        None => PathBuf::from(".").join(target_name),
    }
}
...
fn main() {
    ...
    debug_log(
        debug,
        &format!("[DEBUG]   link_name_or_dir: {:?}", parsed.link_name_or_dir),
    );
    debug_log(
        debug,
    ...
    if parsed.targets.len() > 1 {
        let Some(link_dir) = parsed.link_name_or_dir.as_ref() else {
            exit(0);
        };
    ...
    if parsed.targets.len() == 1 {
        ...
        if let Some(llvmir_source) = find_llvmir_file(target, &llvmir_dir) {
            ...
            let link_name = resolve_single_link(target, parsed.link_name_or_dir.as_deref());
